...
This pipeline's own code lives under `kinect/`. The cMAS and MotionBERT
model code is **not** duplicated here — it is shared from `../models/cMAS`
and `../models/MotionBERT` (also used by `../hrnet`), and is pulled in via
relative-path `sys.path` insertion at the top of each script.

```
final_project/
├── models/                          # Shared model code (used by hrnet/ and kinect/)
│   ├── cMAS/                        # c-MAS diffusion model
...
│   │   └── save/yoga_diffusion_model/  # Pretrained checkpoint (200K steps, git-ignored)
│   └── MotionBERT/                  # Action recognition backbone
│       ├── lib/                     # MotionBERT model / data / utils
│       ├── configs/action/          # Training config YAMLs
│       └── save/MB_ft_NTU60_xsub/   # Pretrained checkpoint (git-ignored)
│
└── kinect/
    ├── data/                        # Shared raw NTU60 3D annotations (git-ignored)
    │   └── ntu60_3danno.pkl
    ├── cmas_pipeline/                # Generative multi-view pipeline (uses ../../models/cMAS)
    │   ├── pipeline.py               # Multi-view generation (--split val/train, --stage 1/2/3/all)
    │   ├── eval.py                    # c-MAS inference evaluation (--mode full/quick/time)
    │   ├── pipeline_out/              # Val generation outputs (npy, pkls, vis) — large dirs git-ignored
    │   └── pipeline_train_out/        # Train generation outputs (npy, pkls) — large dirs git-ignored
    │
    └── motionbert_har/               # Action recognition backbone + fusion classifier (uses ../../models/MotionBERT)
        ...
        ├── eval_motionbert_multiview_baseline.py  # View-set ensemble baseline evaluation
        ├── generate_motionbert_baseline_pkls.py   # Lifting baseline pkl generation
        ├── data/action/               # Preprocessed pkl data (git-ignored)
        │   ├── ntu60_gt_*.pkl          # GT projection data
        │   └── ntu60_cmas_*.pkl        # c-MAS generated data
        └── view_attn_out/
            ├── checkpoints_logit_combo/  # Logit-level MLP checkpoints (git-ignored)
            ├── checkpoints_feat/         # Feature-level MLP checkpoints (git-ignored)
            ├── checkpoints_mixed/        # Mixed GT+c-MAS MLP checkpoints (git-ignored)
            ├── logits/                   # Extracted 60-dim logits (git-ignored)
            └── vis/                      # Result visualization images
```

### Shared Models

`pipeline.py` and `eval.py` (under `cmas_pipeline/`), and `train.py`,
`eval.py`, `eval_motionbert_multiview_baseline.py`, and
`generate_motionbert_baseline_pkls.py` (under `motionbert_har/`) resolve the
model code relative to their own file location, e.g.:
...
Checkpoint/config defaults (`MB_CONFIG`, `MB_WEIGHTS`, c-MAS `model_path`,
the NBA normalization `Mean.npy`/`Std.npy`) all default to paths under
`../../models/`, so the scripts work out of the box once
`final_project/models/MotionBERT` and `final_project/models/cMAS` are
populated with their pretrained weights.
...
- `kinect/motionbert_har/view_attn_out/logits/` — pre-extracted GT and c-MAS logits (`.npz`)
- `kinect/motionbert_har/view_attn_out/checkpoints_logit_combo/` — trained logit-level MLP weights
- `kinect/motionbert_har/view_attn_out/checkpoints_feat/` — trained feature-level MLP weights
- `kinect/motionbert_har/view_attn_out/checkpoints_mixed/` — trained mixed MLP weights
...
| Item | Path | Note |
|---|---|---|
| NTU60 3D raw data | `kinect/data/ntu60_3danno.pkl` | Shared, git-ignored |
| GT projection data (val, ×8 angles) | `kinect/motionbert_har/data/action/ntu60_gt_*.pkl` | Git-ignored |
| c-MAS generated data (val, ×7 angles) | `kinect/motionbert_har/data/action/ntu60_cmas_*.pkl` | Git-ignored |
| c-MAS generated data (train, ×7 angles) | `kinect/motionbert_har/data/action/ntu60_cmas_train_*.pkl` | Git-ignored |
| c-MAS pretrained checkpoint | `models/cMAS/save/yoga_diffusion_model/checkpoint_200000.pth` | Shared with hrnet/, git-ignored (209MB) |
| MotionBERT pretrained checkpoint | `models/MotionBERT/save/MB_ft_NTU60_xsub/best_epoch.bin` | Shared with hrnet/, git-ignored (231MB) |
| Logit-level MLP checkpoints | `kinect/motionbert_har/view_attn_out/checkpoints_logit_combo/` | Git-ignored |
| Feature-level MLP checkpoints | `kinect/motionbert_har/view_attn_out/checkpoints_feat/` | Git-ignored |
| Mixed MLP checkpoints | `kinect/motionbert_har/view_attn_out/checkpoints_mixed/` | Git-ignored |
