...
# ── Paths ─────────────────────────────────────────────────────────────────
NTU_DATA_PATH  = '/workspace/clone_model/data/ntu60_3danno.pkl'
MB_CONFIG      = '/workspace/clone_model/MotionBERT/configs/action/MB_ft_NTU60_xsub.yaml'
MB_WEIGHTS     = '/workspace/clone_model/MotionBERT/save/MB_ft_NTU60_xsub/best_epoch.bin'
...
def run_full(opts):
    ...
    sys.path.insert(0, '/workspace/clone_model/MotionBERT')

    nba_mean = np.load('data_loaders/nba/Mean.npy').astype(np.float32)
    nba_std  = np.load('data_loaders/nba/Std.npy').astype(np.float32)
...
def parse_args():
    ...
    p.add_argument('--model_path',      default='save/yoga_diffusion_model/checkpoint_200000.pth')
